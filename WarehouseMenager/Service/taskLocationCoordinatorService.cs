...
namespace WarehouseMenager.Service
{
    internal class taskLocationCoordinatorService
    {
                ...
                try
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            var updateCommand = new MySqlCommand(@"UPDATE locations SET products_products_id = NULL, IsOnLocation = 0 WHERE locations_id = @LocationId;", connection, transaction);
        ...
        public async Task<bool> DeleteLoadTaskAsync(taskModel task)
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                try
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            deleteCommand.Parameters.AddWithValue("@TaskID", task.Id);
                            deleteCommand.ExecuteNonQuery();
                            var updateCommand = new MySqlCommand(@"UPDATE locations SET IsOnLocation = 1 WHERE locations_id = @LocationId;", connection, transaction);
                            updateCommand.Parameters.AddWithValue("@LocationId", task.Location.Id);
                            updateCommand.ExecuteNonQuery();
                            transaction.Commit();   //commit changes to DB
                            Console.WriteLine("Transaction completed successfully!");
