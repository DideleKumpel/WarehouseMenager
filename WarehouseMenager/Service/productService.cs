...
namespace WarehouseMenager.Service
{
    internal class productService
    {
                    ...
                    using (var command = new MySqlCommand(query, connetion))
                    {
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            ...
                            while (await reader.ReadAsync())
                            {
                                ...
                                product.NumberOfItemsOnShelf = await CountNumberOfItemOnShelf(product.Barcode);
        ...
        public async Task<int> CountNumberOfItemOnShelf(string Barcode)
        {
            using (var connetion = new MySqlConnection(_connectionString))
            {
                try
                {
                    await connetion.OpenAsync();
                    string query = "SELECT COUNT(*) FROM locations WHERE products_products_id = @Barcode AND IsOnLocation = TRUE;";
                    using (var command = new MySqlCommand(query, connetion))
                    {
                        command.Parameters.AddWithValue("@Barcode", Barcode);
                        object result = await command.ExecuteScalarAsync();
